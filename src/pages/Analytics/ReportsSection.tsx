...
const buildWorksheet = (data: any, reportTitle: string, projectName: string): XLSX.WorkSheet => {
  ...
  for (const [k, v] of Object.entries(data)) {
    ...
    else if (v !== null && typeof v === 'object') nestedObjects.push([k, v]);
  ...
  if (scalars.length > 0 || nestedObjects.length > 0) {
    ...
    for (const [k, v] of scalars) rows.push([humanize(k), formatCellValue(v)]);
    for (const [parentKey, obj] of nestedObjects) {
      const flat = flattenObj(obj);
      for (const [k, v] of Object.entries(flat)) rows.push([humanize(`${parentKey}.${k}`), formatCellValue(v)]);
    }
    rows.push([]);
  }
  ...
  for (const [arrayKey, arr] of arrays) {
    ...
    if (typeof arr[0] === 'object' && arr[0] !== null) {
      ...
      for (const flatRow of flatRows) rows.push(headers.map(h => formatCellValue(flatRow[h])));
    } else {
      rows.push(['Value']);
      for (const item of arr) rows.push([formatCellValue(item)]);
    }
    rows.push([]);
  }
...
const generateProjectSummaryExport = async (project: any) => {
  ...
  try {
    ...
    const finRows: any[][] = [['FINANCIAL SUMMARY'], [`Project: ${projectName}`], []];
    if (cost) {
      finRows.push(['COST OVERVIEW'], ['Metric', 'Value']);
      for (const [k, v] of Object.entries(flattenObj(cost))) finRows.push([humanize(k), formatCellValue(v)]);
      finRows.push([]);
    }
    if (evm) {
      finRows.push(['EARNED VALUE METRICS'], ['Metric', 'Value']);
      for (const [k, v] of Object.entries(flattenObj(evm))) finRows.push([humanize(k), formatCellValue(v)]);
      finRows.push([]);
    }
    if (forecast) {
      finRows.push(['FORECAST'], ['Metric', 'Value']);
      for (const [k, v] of Object.entries(flattenObj(forecast))) finRows.push([humanize(k), formatCellValue(v)]);
      finRows.push([]);
    }
    if (finRows.length <= 3) finRows.push(['No financial data available.']);
    const wsFin = XLSX.utils.aoa_to_sheet(finRows);
    autoFitColumns(wsFin);
    XLSX.utils.book_append_sheet(wb, wsFin, 'Financial Summary');

    const riskRows: any[][] = [['RISK SUMMARY'], [`Project: ${projectName}`], []];
    if (risk) {
      ...
      if (Object.keys(summaryFlat).length > 0) {
        ...
        for (const [k, v] of Object.entries(summaryFlat)) riskRows.push([humanize(k), formatCellValue(v)]);
        riskRows.push([]);
      }
      ...
      if (risksArr.length > 0) {
        ...
        for (const r of risksArr) riskRows.push(riskFields.map(f => formatCellValue(r[f])));
        riskRows.push([]);
      }
      ...
      if (mitigations && typeof mitigations === 'object') {
        ...
        for (const [k, v] of Object.entries(flattenObj(mitigations))) riskRows.push([humanize(k), formatCellValue(v)]);
        riskRows.push([]);
      }
    } else {
    ...
    const buf = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const safeName = projectName.replace(/[^a-zA-Z0-9_-]/g, '_').substring(0, 40);
    saveAs(new Blob([buf], { type: 'application/octet-stream' }), `Project_Summary_${safeName}.xlsx`);
    hide();
...
const REPORT_DEFS: ReportDef[] = [
  { key: 'cost_overview', title: 'Cost Overview', description: 'Total budget, costs, expenses, and variance for the project.', icon: <DollarOutlined />, color: '#00ff88', fetch: (id) => costService.getCostOverview(id), sheetName: 'Cost Overview' },
  { key: 'evm', title: 'Earned Value Management (EVM)', description: 'SPI, CPI, Planned Value, Earned Value, and Actual Cost.', icon: <BarChartOutlined />, color: '#1890ff', fetch: (id) => costService.getEVM(id), sheetName: 'EVM' },
  { key: 'cost_forecast', title: 'Cost Forecast', description: 'EAC, ETC, VAC, and TCPI project cost forecast.', icon: <DollarOutlined />, color: '#fa8c16', fetch: (id) => costService.getCostForecast(id), sheetName: 'Cost Forecast' },
  { key: 'risk_report', title: 'Risk Report', description: 'Risk summary, severity distribution, and mitigation status.', icon: <ExclamationCircleOutlined />, color: '#ff4d4f', fetch: (id) => riskService.getRiskReport(id), sheetName: 'Risk Report' },
  { key: 'schedule_overview', title: 'Schedule Overview', description: 'Project schedule, task counts, critical path summary.', icon: <CalendarOutlined />, color: '#722ed1', fetch: (id) => scheduleService.getProjectSchedule(id), sheetName: 'Schedule' },
  { key: 'stakeholder_summary', title: 'Stakeholder Summary', description: 'Stakeholder count, engagement level, and influence breakdown.', icon: <UsergroupAddOutlined />, color: '#eb2f96', fetch: (id) => stakeholderService.getSummary(id), sheetName: 'Stakeholders' },
  { key: 'expense_summary', title: 'Expense Summary', description: 'All project expenses grouped by category and approval status.', icon: <DollarOutlined />, color: '#52c41a', fetch: (id) => costService.getExpenseSummary(id), sheetName: 'Expenses' },
];
...
const renderData = (data: any) => {
  ...
  for (const [parentKey, obj] of nestedEntries) {
    ...
    for (const [k, v] of Object.entries(flat)) {
      if (typeof v !== 'object' || v === null) allScalarEntries.push([`${parentKey} › ${k}`, v]);
    }
  }
  return (
    <Space direction="vertical" style={{ width: '100%' }} size={16}>
          ...
          {allScalarEntries.map(([k, v]) => (
            <Col xs={12} sm={8} md={6} key={k}>
              <Card style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8 }} bodyStyle={{ padding: '12px 14px' }}>
                <Statistic title={<Text style={{ color: '#777', fontSize: 11 }}>{humanize(k)}</Text>} value={String(formatCellValue(v))} valueStyle={{ color: '#ffffff', fontSize: 15, fontWeight: 600 }} />
              </Card>
            </Col>
          ))}
        </Row>
      ...
      {arrayEntries.map(([k, v]) => {
        ...
        const columns = visibleKeys.map(col => ({
          ...
          dataIndex: col,
          key: col,
          render: (val: any) => { const fv = formatCellValue(val); return typeof fv === 'number' ? fv.toLocaleString() : fv; },
        }));
        return (
          <div key={k}>
            <Text style={{ color: '#aaa', fontSize: 13, display: 'block', marginBottom: 8 }}>{humanize(k)}</Text>
            <Table rowKey={(_, i) => String(i)} dataSource={flatRows} columns={columns} pagination={arr.length > 10 ? { pageSize: 10 } : false} size="small" style={{ background: '#141414' }} />
          </div>
        );
      })}
    </Space>
...
const ReportsSection: React.FC = () => {
  ...
  const generateAll = async () => {
    if (!selectedProject?.id) { message.warning('Select a project first'); return; }
    for (const def of REPORT_DEFS) generateReport(def);
  };

  return (
    <div style={{ marginTop: 32 }}>
      <Title level={4} style={{ color: '#fff', marginBottom: 16 }}>Reports</Title>
      <Row justify="space-between" align="middle" style={{ marginBottom: 24 }} wrap>
        ...
        <Col>
          <Space wrap>
            {selectedProject && (
              <>
                <Button icon={<DownloadOutlined />} type="primary" onClick={() => generateProjectSummaryExport(selectedProject)} style={{ background: GREEN, borderColor: GREEN }}>
                  Project Summary
                </Button>
                <Button icon={<ReloadOutlined />} onClick={generateAll} style={{ borderColor: 'rgba(0,153,68,0.5)', color: '#00ff88' }}>
                  Generate All
                </Button>
                <Button
